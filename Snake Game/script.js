...
// Handle Keyboard inputs for sanke movement
...
// Generate a food position that doesn't collide with snake
...
function generateFoodPosition() {
  while (true) {
    ...
    let collisonWithSnake = false;
    for (let segment of snake) {
      if (segment.x === newFoodPosition.x && segment.y === newFoodPosition.y) {
        collisonWithSnake = true;
        break;
      }
    }

    // Returns the position if there is no collision
    if (!collisonWithSnake) {
...
function update() {
  ...
  if (blinkCounter % 4 === 0) {
    ...
    // handle foood collisions with wall
...
// Pasuse the game when window losses focus
